...
namespace kuiper_infer {
    __global__ void adaptiveAvgPoolingLayer_kernel(float *input, float *output, int N) {

    }

    void adaptiveAvgPoolingLayer_gpu_kernel(const std::shared_ptr<Tensor<float>> &input, std::shared_ptr<Tensor<float>> &output) {
    ...
    void AdaptiveAvgPoolingLayer::ForwardsCuda(const std::vector<std::shared_ptr<Tensor<float>>> &inputs,
