...
namespace kuiper_infer {
    __global__ void catLayer_kernel(float *input, float *output, int N) {
//        int idx = threadIdx.x + blockDim.x * blockIdx.x;
//        if (idx < N)output[idx * sizeof(float)] = input[idx * sizeof(float)] > 0 ? input[idx * sizeof(float)] : 0;
    }

    void catLayer_gpu_kernel(const std::shared_ptr<Tensor<float>> &input, std::shared_ptr<Tensor<float>> &output) {
...
//        relu<<<ceil, bs>>>(dev_in, dev_out, input->size());
//        CUDA_CHECK(cudaMemcpy((float *) output->data().mem, dev_out, sizeof(float) * output->size(),
//                              cudaMemcpyKind::cudaMemcpyDeviceToHost));
    }

    void CatLayer::ForwardsCuda(const std::vector<std::shared_ptr<Tensor<float>>> &inputs,
