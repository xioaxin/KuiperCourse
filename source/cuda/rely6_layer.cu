...
namespace kuiper_infer {
    __global__ void relu6Layer_kernel(float *input, float *output, int N, float thresh) {
        int index = threadIdx.x + blockDim.x * blockIdx.x;
        if (index < N)output[index] = MIN_CUDA(MAX_CUDA(input[index], 0.f), thresh);
    }

    void relu6Layer_gpu_kernel(const std::shared_ptr<Tensor<float>> &input, std::shared_ptr<Tensor<float>> &output, float thresh) {
        ...
        relu6Layer_kernel<<<ceil, bs>>>(dev_in, dev_out, input->size(), thresh);
    ...
    void Relu6Layer::ForwardsCuda(const std::vector<std::shared_ptr<Tensor<float>>> &inputs,
        ...
        for (int i = 0; i < batch_size; i++) {
            ...
            relu6Layer_gpu_kernel(inputs.at(i), outputs.at(i), this->op_->get_thresh());
        }
    }
}