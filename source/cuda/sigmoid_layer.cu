...
namespace kuiper_infer {
    __global__ void sigmoidLayer_kernel(float *input, float *output, int N) {
        int index = threadIdx.x + blockDim.x * blockIdx.x;
        if (index < N)output[index] = 1.0 / (1.0 + exp(-input[index]));
    }

    void sigmoidLayer_gpu_kernel(const std::shared_ptr<Tensor<float>> &input, std::shared_ptr<Tensor<float>> &output) {
        ...
        sigmoidLayer_kernel<<<ceil, bs>>>(dev_in, dev_out, input->size());
    ...
    void SigmoidLayer::ForwardsCuda(const std::vector<std::shared_ptr<Tensor<float>>> &inputs,
        ...
        for (int i = 0; i < batch_size; i++) {
            ...
            sigmoidLayer_gpu_kernel(inputs.at(i), outputs.at(i));
        }
    }
}