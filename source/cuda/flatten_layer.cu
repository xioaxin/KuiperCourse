...
namespace kuiper_infer {
    __global__ void flattenLayer_kernel(float *input, float *output, int N) {
    }

    void flattenLayer_gpu_kernel(const std::shared_ptr<Tensor<float>> &input, std::shared_ptr<Tensor<float>> &output) {
...
//        CUDA_CHECK(cudaMemcpy((float *) output->data().mem, dev_out, sizeof(float) * output->size(),cudaMemcpyKind::cudaMemcpyDeviceToHost));
    ...
    void FlattenLayer::ForwardsCuda(const std::vector<std::shared_ptr<Tensor<float>>> &inputs,
