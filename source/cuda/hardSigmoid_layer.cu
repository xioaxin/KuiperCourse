...
namespace kuiper_infer {
    __global__ void hardSigmoidLayer_kernel(float *input, float *output, int N) {
        int index = threadIdx.x + blockDim.x * blockIdx.x;
        if (index < N) {
            output[index] = MIN_CUDA(MAX_CUDA((input[index] + 1.0) / 2.0f, 0), 1.0f);
        }
    }

    void hardSigmoidLayer_gpu_kernel(const std::shared_ptr<Tensor<float>>
                                     &input, std::shared_ptr<Tensor<float>> &output) {
        ...
        hardSigmoidLayer_kernel<<<ceil, bs>>>(dev_in, dev_out, input->size());
    ...
    void HardSigmoidLayer::ForwardsCuda(const std::vector<std::shared_ptr<Tensor<float>>> &inputs,
        ...
        for (int i = 0; i < batch_size; i++) {
            ...
            hardSigmoidLayer_gpu_kernel(inputs.at(i), outputs.at(i));
        }
    }
}