...
namespace kuiper_infer {
    AvgPoolingLayer::AvgPoolingLayer(const std::shared_ptr<RuntimeOperator> &op) : Layer("AvgPooling") {
...
#pragma omp parallel for num_threads(batch_size)
    ...
    std::shared_ptr<Layer> AvgPoolingLayer::CreateInstance(const std::shared_ptr<RuntimeOperator> &op) {
        CHECK(op->op_type_ == OpType::kOperatorAvgPooling);
        std::shared_ptr<Layer> avg_layer = std::make_shared<AvgPoolingLayer>(op);
        return avg_layer;
    }

    void AvgPoolingLayer::Forwards() {
