...
namespace kuiper_infer {
    ...
    sftensor Normalize::forward(const kuiper_infer::sftensor &inputs) {
        ...
        sftensor mean = std::make_shared<ftensor>(inputs->shapes());
        sftensor std = std::make_shared<ftensor>(inputs->shapes());
        for (int i = 0; i < channels; ++i) {
            mean->at(i).fill(max_pixel_value_ * mean_[i]);
            std->at(i).fill(max_pixel_value_ * std_[i]);
        }
        return Tensor<float>::elementDiv(std, Tensor<float>::elementSub(mean, inputs));
    }
}